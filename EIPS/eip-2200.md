...
* goal
  * implements net gas metering
* == [EIP-1283] + [EIP-1706] + 💡structured definition💡
  * Reason: 🧠make it interoperable -- with -- OTHER gas changes (_Example:_ [EIP-1884])

## Abstract

* goal
  * ⭐️net gas metering changes' structured definition | `SSTORE` opcode⭐️
    * -- based on -- information / AVAILABLE | MOST implementations
      * Storage slot’s original value
      * Storage slot’s current value
      * Refund counter

* enable
  * contract storage's NEW usages
  * | contract storage's COMMON usages,
    * reduce excessive gas costs
    * _Example:_ MOST implementation work

## Motivation

* allows
  * Subsequent storage write operations | SAME call frame
    * _Example:_ reentry locks, same-contract multi-send, etc.
  * sub call frame can -- exchange storage information, with -- parent call frame
    * storage information / NOT need to be persistent | OUTSIDE of a transaction
    * _Example:_ sub-frame error codes & message passing, etc.

* TODO: The original definition of EIP-1283 created a danger of a new kind of
reentrancy attacks on existing contracts as Solidity by default grants
a "stipend" of 2300 gas to simple transfer calls
* This danger is
...
Change the definition of EIP-1283 using those variables
* The new
specification, combining EIP-1283 and EIP-1706, will look like
below
* The terms *original value*, *current value* and *new value* are
defined in EIP-1283.
...
* If *new value* is
  ...
  * If *original value* does not equal *current value* (this storage
    slot is dirty), `SLOAD_GAS` gas is deducted
* Apply both of the
    following clauses.
...
negative
* If the implementation uses transaction-wise refund counter,
...
* We don't suffer from the optimization limitation of
  EIP-1087
* EIP-1087 requires keeping a dirty map for storage changes,
  ...
  transaction
* This works well for some implementations, but not for
  others
* After [EIP-658], an efficient storage cache implementation
  ...
  and only commit changes at the end of a block
* For them, it is
...
* It covers all usages for a transient storage
* Clients that are easy
  to implement EIP-1087 will also be easy to implement this
  specification
* Some other clients might require a little bit extra
  refactoring on this
* Nonetheless, no extra memory or processing cost
